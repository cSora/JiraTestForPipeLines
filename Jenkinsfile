            ...
            parallel(){
                stage('Chrome tests'){
                    steps {
                        ...
                        sh "echo \"mvn -Dpassword=${password} -Dusername=${username} '-Duser=Auto Tester 11' -Dbrowser=${params.browser} -DbaseUrl=${params.baseUrl} -Dtimeout=${timeout} -DgridUser=${gridUser} -DgridPassword=${gridPassword} test\" > passwordfile "
                        sh "mvn -Dpassword=${password} -Dusername=${username} -Dbrowser=${params.browser} -DbaseUrl=${params.baseUrl} -Dtimeout=${timeout} -DgridUser=${gridUser} -DgridPassword=${gridPassword} test"
